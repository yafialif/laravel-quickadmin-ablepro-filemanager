    ...
    <div class="col-sm-12">
        ...
        <div class="card">
            <div class="card-header">
                <h5>{{ trans('quickadmin::admin.roles-index-roles_list') }}</h5>
            </div>
            <div class="card-block">
                <div id="nestable-menu" class="m-b-10">
                    {!! link_to_route('users.create', trans('quickadmin::admin.roles-index-add_new'), [], ['class' => 'btn btn-primary waves-effect waves-light m-b-10 m-r-20']) !!}
                </div>
                        ...
                        <table id="dom-jqry" class="table table-striped table-bordered nowrap">
                            ...
                            @foreach ($roles as $role)
                                <tr>
                                    <td>{{ $role->title }}</td>
                                    <td>
                                        {!! link_to_route('users.edit', trans('quickadmin::admin.users-index-edit'), [$role->id], ['class' => 'btn btn-sm btn-info']) !!}
                                        {!! Form::open(['style' => 'display: inline-block;', 'method' => 'DELETE', 'onsubmit' => 'return confirm(\'' . trans('quickadmin::admin.users-index-are_you_sure') . '\');',  'route' => array('users.destroy', $role->id)]) !!}
