                ...
                <tbody>
                    @foreach ($recording as $row)
                        <tr>
                            ...
                            <td>{{ isset($row->ustad->nama_ustad) ? $row->ustad->nama_ustad : '' }}</td>
<td>{{ $row->id_recording }}</td>
<td>{{ $row->record }}</td>
<td>{{ $row->note }}</td>
<td>{{ $row->status }}</td>

                            <td>
                                {!! link_to_route(config('quickadmin.route').'.recording.edit', trans('quickadmin::templates.templates-view_index-edit'), array($row->id), array('class' => 'btn btn-sm btn-info')) !!}
            ...
            {!! Form::open(['route' => config('quickadmin.route').'.recording.massDelete', 'method' => 'post', 'id' => 'massDelete']) !!}
